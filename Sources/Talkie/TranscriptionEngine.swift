...
actor TranscriptionEngine {
    ...
    /// language-correction path) and return every non-empty result paired with
    /// its locale. Runs sequentially: this only fires when the first transcript
    /// wasn't a confident match, and the candidate set is small (the user's
    /// non-primary languages). Keeping the loop inside the actor lets the same
    /// buffers be replayed per candidate without re-sending non-`Sendable` audio
    ...
    func transcribeScored(
        ...
        guard SpeechTranscriber.isAvailable, !buffers.isEmpty else {
            NSLog("TalkieLang reTx[\(id)]: bail — unavailable or no buffers (\(buffers.count))")
            return nil
        }
        guard let loc = await SpeechTranscriber.supportedLocale(equivalentTo: Locale(identifier: id)) else {
            NSLog("TalkieLang reTx[\(id)]: bail — locale not supported on this device")
            return nil
        }
        ...
        if await AssetInventory.status(forModules: [transcriber]) != .installed {
            guard installIfNeeded else {
                NSLog("TalkieLang reTx[\(id)]: bail — model not installed (no inline install)")
                return nil
            }
            do {
                try await ensureModelInstalled(for: transcriber)
            } catch {
                NSLog("TalkieLang reTx[\(id)]: bail — model install failed: \(error.localizedDescription)")
                return nil
            }
        }
        guard let format = await SpeechAnalyzer.bestAvailableAudioFormat(compatibleWith: [transcriber]) else {
            NSLog("TalkieLang reTx[\(id)]: bail — no compatible audio format")
            return nil
        }
        ...
        guard !feedBuffers.isEmpty else {
            NSLog("TalkieLang reTx[\(id)]: bail — resample produced no buffers")
            return nil
        }
        ...
        // Accumulate committed text and per-word confidence together.
        let reader = Task { () -> (text: String, confSum: Double, confCount: Int) in
            var text = ""
            var confSum = 0.0
            var confCount = 0
            do {
        ...
        do {
            ...
            reader.cancel()
            NSLog("TalkieLang reTx[\(id)]: bail — analyzer.start threw: \(error.localizedDescription)")
            return nil
        }
        ...
        guard !text.isEmpty else {
            NSLog("TalkieLang reTx[\(id)]: empty transcript")
            return nil
        }
